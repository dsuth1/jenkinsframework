...
ARG JENKINS_VERSION
ENV JENKINS_VERSION ${JENKINS_VERSION:-2.132}
...
COPY plugins.sh /usr/local/bin/plugins.sh
COPY install-plugins.sh /usr/local/bin/install-plugins.sh 
RUN /usr/local/bin/install-plugins.sh workflow-aggregator blueocean ansible pipeline-utility-steps azure-cli azure-credentials
